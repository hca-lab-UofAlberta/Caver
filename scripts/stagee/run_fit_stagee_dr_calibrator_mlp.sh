...
usage() {
  cat <<'EOF'
Usage:
  run_fit_stagee_dr_calibrator_mlp.sh [args...]

This wrapper runs the proposal-side Stage-E DR calibrator fitter inside the
...
if [ "${1}" = "-h" ] || [ "${1}" = "--help" ]; then
  ...
  "${CAVER_REPO_ROOT}/scripts/env/with_openpi_pistepnft_libero_train.sh" -- \
    python "${CAVER_REPO_ROOT}/scripts/stagee/fit_stagee_dr_calibrator_mlp.py" --help
  exit 0
fi

exec "${CAVER_REPO_ROOT}/scripts/env/with_openpi_pistepnft_libero_train.sh" -- \
  python "${CAVER_REPO_ROOT}/scripts/stagee/fit_stagee_dr_calibrator_mlp.py" "$@"

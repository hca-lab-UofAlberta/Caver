...
usage() {
  cat <<'EOF'
Usage:
  submit_stage0_caver_round.sh [options]
...
stamp="$(timestamp_utc)"
run_id="$(make_run_id "stageE" "caver-round" "${task_token}" "${seed}" "${budget_token}" "${stamp}")"
job_name="caver-stageE-caver"
...
job_cmd=(
  "${CAVER_REPO_ROOT}/scripts/stagee/run_stage0_caver_round.sh"
